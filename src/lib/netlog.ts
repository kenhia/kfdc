...
// Deleted here, deliberately: `lineHref`, a kind → path map that both this
// feed and the Ticker linked through. korg now resolves a node id to its own
// page (`/n/:id`), so there is nothing left for kfdc to know — see
// $lib/korglink, which is where every korg URL on the board is built, and
// GP-16 for why a consumer may not keep such a map at all. Both feeds link
// every kind now; neither degrades to plain text any more.
